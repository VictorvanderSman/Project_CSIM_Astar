...
public class PFMultiWiki : MonoBehaviour
{
    ...
    Hashtable obstacles;
    Dictionary<int2, Node> starts, end2;
    Node end;
    int safeGuard = 1000;
    ...
    void Update()
    {
        ...
        if (timePassed > 1f)
        {
            ...
            UnityEngine.Debug.Log(endTime - startTime);
    ...
    void PlaceEndTime()
    {
        ...
        int rndx = UnityEngine.Random.Range(-200, 200);
        int rndy = UnityEngine.Random.Range(-200, 200);

        int2 coord;
        coord.x = rndx; coord.y = rndy;


        UnityEngine.Vector3Int myVector3Int = new UnityEngine.Vector3Int(coord.x, coord.y, 0);
        ...
        if (!obstacles.ContainsKey(coord) && !starts.ContainsKey(coord))
        {
            ...
            map.SetColor(myVector3Int, Color.red);
        }
    }
    ...
    void PlaceEndRandom()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            Node startNode = new Node { coord = int2.zero, parent = int2.zero, gScore = float.MaxValue, hScore = float.MaxValue };

            end2.Add(coord, startNode);
    ...
    void PlaceStartRandom()
    {


        for (int i = 0; i < 1000; i++)
        {
            int rndx = UnityEngine.Random.Range(-299, 299);
            int rndy = UnityEngine.Random.Range(-299, 299);
            ...
            Node startsNode = new Node { coord = coord, parent = int2.zero, gScore = float.MaxValue, hScore = float.MaxValue };

            starts.Add(coord, startsNode);

            map.SetTile(myVector3Int, defaultTile);
            map.SetTileFlags(myVector3Int, TileFlags.None);
            map.SetColor(myVector3Int, Color.green);
        }
    ...
    void PlaceCubeObstacle()
    {
        int2 coord2;

        for (int i = -100; i < 100; i++)
        {
            coord2.x = 100; coord2.y = i;
